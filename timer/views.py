...
def timer_stop(request):
    ...
    # 최근에 시작된 타이머를 찾습니다.
    try:
        recent_timer = Timer.objects.filter(user=user).order_by('-start_time').first()

        if not recent_timer:
            return Response({"detail": "시작된 타이머가 없습니다."}, status=status.HTTP_404_NOT_FOUND)

        recent_timer.stop_time = validated_data.get('stop_time')
        recent_timer.duration = validated_data.get('stop_time') - recent_timer.start_time
        recent_timer.save()
...
def timer_rest(request):
    ...
    try:
        ...
        recent_timer.duration = validated_data.get('rest_time') - recent_timer.start_time
        recent_timer.save()
